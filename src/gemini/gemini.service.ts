...
export class GeminiService {
  ...
  async executeTask(dto: {
    ...
    switch (dto.task) {
      case 'analyze-video': {
        if (!dto.videoData || !dto.mimeType) throw new BadRequestException('videoData and mimeType required');
        const m = dto.metrics;
  ...
  private buildScriptPrompt(analysisResult: string, transcript: string | null, hookType: string | null): string {
