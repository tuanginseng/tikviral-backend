...
export class AffiliateService {
  ...
  async syncKalodataProducts(kalodataCookie: string) {
    if (!kalodataCookie) {
      throw new BadRequestException('Kalodata cookie is required');
    }
    ...
    if (!kalodataResponse.ok) {
      throw new InternalServerErrorException(`Kalodata API failed: ${kalodataResponse.status}`);
    }

    const kalodataData = await kalodataResponse.json();
    if (!kalodataData.success || !kalodataData.data || !Array.isArray(kalodataData.data)) {
      throw new InternalServerErrorException('Invalid response from Kalodata API');
    }
    ...
    const productsToUpsertMap = new Map<string, any>();
    const kalodataProductNames = new Set<string>();

    const parseRevenue = (saleValue: any): number | null => {
      if (saleValue === null || saleValue === undefined) return null;
    ...
    for (const kp of kalodataProducts) {
      if (kp.product_title && kp.sale) {
        ...
        if (typeof kp.commission_rate === 'string' && kp.commission_rate.includes('%')) {
          ...
          if (!isNaN(parsed)) commissionPercentage = parsed;
    ...
    if (upsertError) throw new InternalServerErrorException(upsertError.message);
    ...
    switch (action) {
      case 'get-all': {
        ...
        if (order && Array.isArray(order)) {
          order.forEach((sOption: any) => { query = query.order(sOption.column, { ascending: sOption.ascending }); });
        } else {
      ...
      case 'update-positions': {
        const results = await Promise.all(payload.updates.map((item: any) =>
          supabaseAdmin.from('affiliate_products').update({ position: item.position }).eq('id', item.id)
        ));
        if (results.some(r => r.error)) throw new InternalServerErrorException('Failed to update some positions');
  ...
  async manageCommissions(dto: any) {
    ...
    switch (action) {
      case "list": {
        let query = admin.from("affiliate_commissions").select(
          "id, referrer_id, referral_id, transaction_id, amount, commission_rate, commission_amount, status, created_at, paid_at",
          { count: "exact" }
        ...
        const referrerIds = [...new Set((rows ?? []).map((r: any) => r.referrer_id))];
      ...
      case "summary": {
        ...
        const byReferrer: any = {};
        for (const c of commissions ?? []) {
          const id = c.referrer_id;
          if (!byReferrer[id]) byReferrer[id] = { referrer_id: id, pending: 0, paid: 0, count_pending: 0, count_paid: 0 };
          const amt = Number(c.commission_amount);
          if (c.status === "pending") {
            byReferrer[id].pending += amt; byReferrer[id].count_pending += 1;
          } else if (c.status === "paid") {
            byReferrer[id].paid += amt; byReferrer[id].count_paid += 1;
          }
        }
      ...
      case "mark-paid": {
        if (!Array.isArray(commission_ids) || commission_ids.length === 0) throw new BadRequestException("commission_ids required");
  ...
  async recordReferral(token: string, refCode: string) {
    const admin = this.supabaseService.getAdminClient();
    ...
    if (affiliateRow.user_id === user.id) return { success: false, message: "Không thể dùng mã của chính mình" };
    ...
    if (existing) return { success: false, message: "Tài khoản đã được gắn giới thiệu trước đó" };
  ...
  async uploadAffiliateImage(imageData: string, mimeType: string, fileName: string) {
    const admin = this.supabaseService.getAdminClient();
